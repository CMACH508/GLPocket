...
class TransformerBTS(nn.Module):
    ...
    def encode(self, x):
        if self.conv_patch_representation:
            # combine_36 embedding with conv patch distribution
            x1_1, x2_1, x3_1, x = self.Unet(x)
            x = self.bn(x)  # [1, 128, 16, 16, 16]
            x = self.relu(x)
            x = self.conv_x(x)  # [1, 512, 16, 16, 16]
            x = x.permute(0, 2, 3, 4, 1).contiguous()  # [1, 16, 16, 16, 512]
            x = x.view(x.size(0), -1, self.embedding_dim)  # [1, 4096, 512]
        ...
        x = self.pe_dropout(x)  # [1, 4096, 512]

        # apply transformer
        x, intmd_x = self.transformer(x)  # x [1, 4096, 512]
        x = self.pre_head_ln(x)
        print('encoder_output=', x.shape)  # [1, 4096, 512]
...
class BTS(TransformerBTS):
    ...
    def decode(self, x1_1, x2_1, x3_1, x, intmd_x, intmd_layers=[1, 2, 3, 4]):
        ...
        print('enter decoder --------------')
        print('x8_0', x8.shape)  # p[1, 4096, 512]
        x8 = self._reshape_output(x8)  # p[1, 512, 16, 16, 16]

        x8 = self.Enblock8_1(x8)  # p[1, 128, 16, 16, 16]
        x8 = self.Enblock8_2(x8)  # p[1, 128, 16, 16, 16]

        print('00', x8.shape, x3_1.shape)  # [1, 128, 16, 16, 16] [1, 64, 32, 32, 32]
        y4 = self.DeUp4(x8, x3_1)  # [1, 128, 16, 16, 16] [1, 64, 32, 32, 32] -> (1, 64, 32, 32, 32)
        y4 = self.DeBlock4(y4)

        y3 = self.DeUp3(y4, x2_1)  # [1, 64, 32, 32, 32] [1, 32, 64, 64, 64] -> (1, 32, 64, 64, 64)
        y3 = self.DeBlock3(y3)
...
class DeUp_Cat(nn.Module):
    ...
    def forward(self, x, prev):
        # print('cat:', x.shape, prev.shape)  # cat: torch.Size([1, 128, 16, 16, 16]) torch.Size([1, 64, 32, 32, 32])
        x1 = self.conv1(x)
        y = self.conv2(x1)
        # y = y + prev
